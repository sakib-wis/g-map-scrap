...
function startTimer() {
  ...
  timer = setInterval(() => {
    elapsed++;
    countTime.textContent = elapsed < 60 ? elapsed + 's' : Math.floor(elapsed/60) + 'm' + (elapsed%60) + 's';
  }, 1000);
}
...
async function injectAndStart(tabId) {
  return new Promise(resolve => {
    chrome.scripting.executeScript({ target: { tabId }, files: ['content.js'] }, () => {
      if (chrome.runtime.lastError) { log('Inject error: ' + chrome.runtime.lastError.message, 'error'); resolve(false); return; }
      setTimeout(() => {
        chrome.tabs.sendMessage(tabId, { action: 'START_SCRAPE' }, res => {
          if (chrome.runtime.lastError) { log('Message error: ' + chrome.runtime.lastError.message, 'error'); resolve(false); }
          else resolve(true);
        });
      }, 300);
    });
...
btnClear.addEventListener('click', () => {
  ...
  chrome.storage.local.set({ gmapResults: [] });
  updateStats(0, 0); countTime.textContent = '0s';
  progressBar.style.width = '0%'; progressLabel.textContent = 'Ready';
  btnExcel.disabled = true; btnCsv.disabled = true;
  setStatus('', 'Data cleared.'); log('Cleared.', 'warn');
});
...
btnCsv.addEventListener('click', () => {
  ...
  const rows = results.map(r => COLUMNS.map(c => escapeCsv(r[c.key])).join(','));
  ...
  log(`CSV exported: ${results.length} records × ${COLUMNS.length} columns.`, 'success');
});
...
btnExcel.addEventListener('click', () => {
  ...
  // Column widths
  const colWidths = [180, 120, 55, 70, 90, 60, 110, 150, 220, 90, 100, 70, 70, 160, 200];
  ...
  const dataRows = results.map((r, i) => {
    ...
    const cells  = COLUMNS.map((c, ci) => {
      ...
      // Status column
      if (c.key === 'status') {
        const sStyle = /open now|open 24/i.test(val) ? 'open' : /closed/i.test(val) ? 'closed' : base;
...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
  ...
  const a = Object.assign(document.createElement('a'), {
    href: URL.createObjectURL(new Blob([xml], { type: 'application/vnd.ms-excel;charset=utf-8;' })),
    download: `gmapscrap_${new Date().toISOString().slice(0,10)}.xls`
  });
  a.click(); URL.revokeObjectURL(a.href);
  log(`Excel exported: ${results.length} records × ${COLUMNS.length} columns.`, 'success');
});
...
chrome.runtime.onMessage.addListener(msg => {
  ...
  if (msg.type === 'DONE') {
    ...
    if (results.length > 0) {
      ...
      log(`Fields: Name, Category, Rating, Reviews, Status, Phone, Website, Address, City, State, Pincode, Hours`, '');
    } else {
      log('0 records. Make sure Google shows the local business map panel.', 'warn');
    }
  }
